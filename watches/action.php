...
if (isset($_POST['action'])) {
    ...
    if (isset($_POST['screen_size'])) {
        ...
        $whereClause .= " AND sm_watch_specs.screen_size IN ('" . $screen_size . "')";
    }
    if (isset($_POST['os'])) {
        $os = implode("','", $_POST['os']);
        $whereClause .= " AND sm_watch_specs.os IN ('" . $os . "')";
    }
    if (isset($_POST['compatible_os'])) {
        $compatible_os = implode("','", $_POST['compatible_os']);
        $whereClause .= " AND sm_watch_specs.compatible_os IN ('" . $compatible_os . "')";
    }
    // page nos. logic
    ...
    INNER JOIN sm_watch_specs ON products.product_id = sm_watch_specs.product_id INNER JOIN brands ON products.brand_id = brands.brand_id  WHERE products.status = 1 " . $whereClause
    );
